...
public:
    ...
    */
            ...
            int& beam_number)
    {
       ...
       tf::Vector3 temp_position , base_position, inc_position, un_position;
       double temp_yaw , base_yaw, inc_yaw;
       tf::Matrix3x3 T_base_inverse, T_temp, T_b_t;

       beam_number--;

       base_position = frame_base_pose.getOrigin();
       base_yaw = tf::getYaw(frame_base_pose.getRotation()); // radian
       T_base_inverse = tf::Matrix3x3(cos(base_yaw),-sin(base_yaw),base_position.getX(),sin(base_yaw),cos(base_yaw),base_position.getY(),0,0,1).inverse();

       inc_position = (frame_start_pose.getOrigin() - frame_end_pose.getOrigin()) / beam_number;
       inc_yaw = (tf::getYaw(frame_start_pose.getRotation()) - tf::getYaw(frame_start_pose.getRotation())) / beam_number;

       for(int i=0; i<=beam_number; i++)
       {
           temp_position = frame_start_pose.getOrigin() - inc_position * i;
           temp_yaw = tf::getYaw(frame_start_pose.getRotation()) - inc_yaw * i;

           T_temp = tf::Matrix3x3(cos(temp_yaw),-sin(temp_yaw),temp_position.getX(),sin(temp_yaw),cos(temp_yaw),temp_position.getY(),0,0,1);
           T_b_t = T_base_inverse * T_temp;

           double x = ranges[startIndex + i] * cos(angles[startIndex + i]);
           double y = ranges[startIndex + i] * sin(angles[startIndex + i]);
           
           un_position = T_b_t * tf::Vector3(x,y,1);

           ranges[startIndex + i] = sqrt(un_position.x()*un_position.x() + un_position.y()*un_position.y());
           angles[startIndex + i] = atan2(un_position.y(),un_position.x());
       }

       //ROS_INFO("beam_number : %d",beam_number) ;
       ROS_INFO("T_b_t : %f, %f " , T_b_t.getColumn(2).x(), T_b_t.getColumn(2).y() );
